...
class DefaultSignaling:
    def __init__(self, serverUrl):
        ...
        logger.info('__init__ serverUrl=%s broadcasterId=%s' %(self.serverUrl, self.broadcasterId))
        # check room
        r = requests.get(self.serverUrl, verify=False)
        r.raise_for_status()
        # logger.debug('room: %s', r.json())
        # create broadcaster
        r = requests.post(self.serverUrl + '/broadcasters', json={
    ...
    def stop(self):
        logger.info('stop')
        r = requests.delete(self.serverUrl + '/broadcasters/' + self.broadcasterId, verify=False)
        r.raise_for_status()
    ...
    def do_createPlainRtpTransport(self, data, cb):
        ...
        r = requests.post(self.serverUrl + '/broadcasters/' + self.broadcasterId + '/transports', 
    ...
    def do_transportProduce(self, data, cb):
        ...
        r = requests.post(self.serverUrl + '/broadcasters/' + self.broadcasterId 
